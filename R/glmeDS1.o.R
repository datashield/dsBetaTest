...
#' @export
glmeDS1.o <- function(formula, family, weights, data){
  ...
  mod.glmer.ds <- glmer(formula2use, family=family, contrasts=NULL, data=dataTable)
  ...
  print(summary(mod.glmer.ds))
  X.mat <- as.matrix(mod.glmer.ds$x)
  
  dimX <- dim((X.mat))
  
  y.vect <- as.vector(mod.glmer.ds$y)
